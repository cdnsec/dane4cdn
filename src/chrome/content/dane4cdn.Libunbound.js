...
dane4cdn.Libunbound = {
  ...
  init: function() {
  	AddonManager.getAddonByID("dane4cdn@ccert.edu.cn", function(addon) {
        ...
        dane4cdn.Extension.logMsg("OS: " + os + " ABI: " +abi);
		...
		try {
			...
			if(os.match("Darwin")) {
				...
				dane4cdn.Extension.logMsg("Unsupported OS");

				//XXX Gracefully exit and display something to the user.
			}

			dane4cdn.Libunbound._init(libunboundFile);
		}
		catch(e) {
			...
			if(os.match("Darwin")) {
				...
				var libunboundFile = addon.getResourceURI("components/libunbound64.so.2.1.5")
					.QueryInterface(Components.interfaces.nsIFileURL).file.path;
				//Libunbound._init(libunboundFile.path);
			} else if(os.match("Linux") && abi.match("x86")) {
				var libunboundFile = addon.getResourceURI("components/libunbound.so.2.1.5")
						.QueryInterface(Components.interfaces.nsIFileURL).file.path;
			} else if(os.match("WINNT")) {
  ...
  _init: function(libunboundFile) {
    dane4cdn.Extension.logMsg("Loading libunbound from: "+libunboundFile);
    ...
    var fwd = dane4cdn.Extension.prefs.getCharPref("dnsforwarder");
    if(fwd != "") {
       dane4cdn.Extension.logMsg("Using dns forwarder:"+fwd);
       this.ub_ctx_set_fwd(this.ctx,fwd);	
    }
    
    //load the rootanchor from preferences
    var rootanchor = dane4cdn.Extension.prefs.getCharPref("rootanchor");
    ...
    var dlvanchor = dane4cdn.Extension.prefs.getCharPref("dlvanchor");
    if(dlvanchor == "") {
    ...
    // this.ub_ctx_set_option(this.ctx, "val-log-level:","5");
  },
